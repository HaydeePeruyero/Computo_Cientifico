...
La consulta específica ordena los resultados primero en orden ascendente según la columna `taken`, que representa el número de visita o una marca de tiempo de la medición. Luego, dentro de cada grupo de valores iguales en la columna `taken`, los resultados se ordenan en orden descendente por la columna `person`, que contiene los nombres de los científicos.
...
2. **MAX**: Encuentra el valor máximo en una columna.
   
```sql
SELECT max(dated) FROM Visited;
```

3. **AVG**: Calcula el promedio de los valores en una columna.
...
4. **COUNT**: Cuenta el número de filas en un conjunto de resultados.
...
b) ¿Pueden encontrar una manera de listar todos los apellidos de los científicos separados por una coma? ¿Pueden encontrar una manera de listar todos los nombres personales y apellidos de los científicos separados por una coma?
...
![Datos relacionales](D:/Users/hayde/Documents/R_sites/Computo_Cientifico/img/sql-join-structure.svg)
...
### Paso 1: Crear una base de datos y conectarla
...
Una vez que hayas definido la estructura de la nueva tabla, puedes ejecutar el comando `CREATE TABLE` en SQLite3 para crear la tabla en tu base de datos existente.
...
2. UPDATE: Este comando se utiliza para actualizar registros existentes en una tabla.
...
3. DELETE: Este comando se utiliza para eliminar registros de una tabla.
...
```sql
INSERT INTO Estudiantes (nombre, edad, promedio) VALUES ('Roberta', 20, 9.5);
INSERT INTO Estudiantes (nombre, edad, promedio) VALUES ('Diego', 22, 8.9);

INSERT INTO Cursos (nombre, profesor) VALUES ('Matemáticas', 'Profesor A');
INSERT INTO Cursos (nombre, profesor) VALUES ('Historia', 'Profesor B');
```

Esto insertará dos registros en la tabla `Estudiantes` y dos registros en la tabla `Cursos`.
...
2. Seleccionar los cursos y sus profesores:

```sql
SELECT nombre, profesor FROM Cursos;
```

3. Seleccionar estudiantes mayores de 21 años:

```sql
SELECT * FROM Estudiantes WHERE edad > 21;
```

4. Seleccionar estudiantes cuyo promedio sea mayor o igual a 8.0:
